...
def write_parameters_info_to_file_yaml(config_file, mesh_file, start_time, end_time, name_of_script_yaml):
    ...
    if boundary_conditions_have_equal_displacement == "yes":
        summary = [
            ...
            f"  - Young modulus of rock: {config.get_young_modul_rock_gpa()} GPa ({config.get_young_modul_rock_gpa()*1000000000} Pa)",
            f"  - Young modulus of fractures: {config.get_young_modul_rock_gpa()/config.get_reduction_value_for_fractures()} GPa ({config.get_young_modul_rock_gpa()*1000000000/config.get_reduction_value_for_fractures()} Pa)",
            "-" * 70,
            "ADDITIONAL INFORMATION",
            f"  - Process started at: {start_time_str}",
            f"  - Process finished at: {end_time_str}",
        ...
        summary = [
            ...
            f"  - Cross section: {config.get_cross_section_multiplier() * rectangle_dimensions[0] * rectangle_dimensions[1]}",
            f"  - Young modulus of rock: {config.get_young_modul_rock_gpa() * 1000000000} Pascals",
            f"  - Young modulus of fractures: {config.get_young_modul_rock_gpa() * 1000000000 / config.get_reduction_value_for_fractures()} Pascals",
            "-" * 70,
            "ADDITIONAL INFORMATION",
            f"  - Process started at: {start_time_str}",
            f"  - Process finished at: {end_time_str}",
